...
class AuthService {
  ...
  Future<void> init() async {
    try {
      ...
      // Tenta recuperar token existente
      if (_authStorage.hasValidToken()) {
        _currentAuthResponse = _authStorage.getAuthResponse();
        debugPrint('✅ Token válido encontrado no armazenamento local');
        return;
      }
      
      // Se não houver token, realiza login automático
      debugPrint('🔄 Nenhum token encontrado, realizando login automático...');
